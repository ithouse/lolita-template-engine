class Lolita::TemplateEngineController < Lolita::RestController
  ...
  def engine_current_layout
    @engine_current_layout ||= engine_current_theme && engine_current_theme.layouts.layout(resource.name.to_sym)
  end
  
  def existing_content_blocks
    engine_current_theme && engine_current_theme.content_blocks || []
  end
